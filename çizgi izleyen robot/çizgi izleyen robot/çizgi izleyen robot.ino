...
void loop() {
  ...
  if (digitalRead(SensorSol) == 1 && digitalRead(SensorOrta) == 0 && digitalRead(SensorSag) == 0) { // Sağ sensör çizgiyi gördüğünde robot sağa dönsün.
    sag();
  }
  if (digitalRead(SensorSol) == 0 && digitalRead(SensorOrta) == 0 && digitalRead(SensorSag) == 1) { // Sol sensör çizgiyi gördüğünde robot sola dönsün.
    sol();
  }
...
void ileri() { // Robotun ileri yönde hareketi için fonksiyon tanımlıyoruz.
  ...
  analogWrite(MotorRE, 170); // Sağ motorun hızı 150
  digitalWrite(MotorL1, HIGH); // Sol motorun ileri hareketi aktif
  digitalWrite(MotorL2, LOW); // Sol motorun geri hareketi pasif
  analogWrite(MotorLE, 170); // Sol motorun hızı 150
}
